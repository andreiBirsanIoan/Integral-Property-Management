...
function App() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#0D1B2A', padding: '48px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        ...
        <div>
          <h1 style={{ fontSize: '28px', fontWeight: '500', color: '#E8EEF2', lineHeight: '1.4', marginBottom: '32px' }}>
            {pagina === 'login' ? 'Gestionează apartamente, chiriași și facturi dintr-un singur loc.' : 'Creează-ți contul și începe să gestionezi proprietățile tale.'}
          </h1>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ color: '#4A8FA8' }}>🏠</span><span style={{ fontSize: '13px', color: '#8AAFC2' }}>Management complet apartamente</span></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ color: '#4A8FA8' }}>📄</span><span style={{ fontSize: '13px', color: '#8AAFC2' }}>Facturi și plăți automate</span></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ color: '#4A8FA8' }}>🔧</span><span style={{ fontSize: '13px', color: '#8AAFC2' }}>Tichete de mentenanță</span></div>
          </div>
        </div>
      ...
      {/* DREAPTA */}
      <div style={{ background: '#111E2D', padding: '48px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '20px' }}>
        ...
        {pagina === 'login' ? (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#0D1B2A', borderRadius: '8px', overflow: 'hidden', border: '0.5px solid #1A3A50' }}>
              <button onClick={() => setRol('chirias')} style={{ padding: '12px', border: 'none', cursor: 'pointer', background: rol === 'chirias' ? '#2A7F9E' : 'transparent', color: rol === 'chirias' ? '#E8EEF2' : '#8AAFC2', fontSize: '12px', fontWeight: '500' }}>👤 Chiriaș</button>
              <button onClick={() => setRol('proprietar')} style={{ padding: '12px', border: 'none', cursor: 'pointer', background: rol === 'proprietar' ? '#2A7F9E' : 'transparent', color: rol === 'proprietar' ? '#E8EEF2' : '#8AAFC2', fontSize: '12px', fontWeight: '500' }}>🏠 Proprietar</button>
            </div>
            <div>
          ...
          <>
            <div>
              <h2 style={{ fontSize: '24px', fontWeight: '500', color: '#E8EEF2' }}>Creează cont</h2>
              <p style={{ fontSize: '13px', color: '#8AAFC2', marginTop: '6px' }}>Completează datele de mai jos</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#0D1B2A', borderRadius: '8px', overflow: 'hidden', border: '0.5px solid #1A3A50' }}>
              <button onClick={() => setRol('chirias')} style={{ padding: '12px', border: 'none', cursor: 'pointer', background: rol === 'chirias' ? '#2A7F9E' : 'transparent', color: rol === 'chirias' ? '#E8EEF2' : '#8AAFC2', fontSize: '12px', fontWeight: '500' }}>👤 Chiriaș</button>
              <button onClick={() => setRol('proprietar')} style={{ padding: '12px', border: 'none', cursor: 'pointer', background: rol === 'proprietar' ? '#2A7F9E' : 'transparent', color: rol === 'proprietar' ? '#E8EEF2' : '#8AAFC2', fontSize: '12px', fontWeight: '500' }}>🏠 Proprietar</button>
